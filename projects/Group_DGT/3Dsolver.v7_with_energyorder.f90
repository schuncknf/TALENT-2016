...
!      INTEGER, PARAMETER :: dp = SELECTED_REAL_KIND(12)
      ...
      integer :: Nodemax, kpot, numN2
      REAL(kind=dp) :: Etrial, Eexp
      integer :: points, ifail, converg, i, charge, l, jj, iii, jmin, jmax
      integer :: Nodecount, numN, numZ, strval, n_rad, n_prin, min_index
      REAL(kind=dp) :: a1, a2, a3, pot, normal, deltae, pos
      REAL(kind=dp) :: potV, rProt, valueR0
      REAL(kind=dp) , ALLOCATABLE, DIMENSION(:) :: trialwf
      REAL(kind=dp) , ALLOCATABLE, DIMENSION(:) :: pott
      REAL(kind=dp) , ALLOCATABLE, DIMENSION(:,:) :: corrEorder
      CHARACTER(LEN=5):: Nodecc
      CHARACTER(LEN=5) :: string
      integer, dimension(:,:), allocatable :: levelmatrix 
      integer, dimension(:), allocatable :: energyarray 
!==========     set initial values     ==============================
      Eup=100.E0_dp
      Edown=-100.E0_dp
      epsil=1.E-6_dp
...
!         trialwf(:)=0.0E0_dp
         ...
         allocate(levelmatrix(0:numN2, 0:3), stat = ifail) !change this if N can be less than Z
         if (ifail .ne. 0) STOP

	 levelmatrix(0:numN2,3)=Eup
         energyarray(:)=Eup

         allocate(energyarray(0:numN2), stat = ifail) !change this if N can be less than Z
         if (ifail .ne. 0) STOP
  

         allocate(corrEorder(0:numN2, 0:3), stat = ifail) !change this if N can be less than Z
         if (ifail .ne. 0) STOP
...
! normalized wavefunction
         ...
         if(trialwf(points-1)*trialwf(points) .lt. 0.0E0_dp) &
             Nodecount= Nodecount-1 
         print'(" nodes= ", i3, "  n_rad= ", i3, "  l=", i3, "  2j=", i3)', &
...
!
         if ( iii < 10 ) then
           write( Nodecc, '("0000",i1)' ) iii
         else if ( iii .ge. 10 .and. iii .lt. 100 ) then
           write( Nodecc, '("000",i2)' ) iii
         else if ( iii .ge. 100 .and. iii .lt. 1000 ) then
           write( Nodecc, '("00",i3)' ) iii
         else if ( iii .ge. 1000 .and. iii .lt. 10000 ) then
           write( Nodecc, '("0",i4)' ) iii
         else
           write( Nodecc, '(i5)' ) iii
         end if

         OPEN(UNIT=7, FILE='wf^2'//Nodecc//'.dat', status='unknown')
        ...
        levelmatrix(iii,0) = Nodecount
        levelmatrix(iii,1) = l
        levelmatrix(iii,2) = jj
        levelmatrix(iii,3) = Etrial
...
!=============================================================================
!=================================================================
	 energyarray(:) = levelmatrix(:,3)
         do i=0, numN, 1
             min_index = MINLOC(energyarray,dim=1)
             corrEorder(i,:) = levelmatrix(min_index-1,:)
             energyarray(min_index-1)=Eup
         end do
!====================================================================
!=============================================================
