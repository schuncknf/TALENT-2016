...
!MAIN PROGRAM:
     program numerov
!Used physical constants are in a separate module:
      use variables 
      implicit none
...
! Opens a file and writes potential on it with meshpoints
...
!         end if
         !loading the potential
           ...
           Edown=Edown0
!
! If l is greater than 5 we start calculating wavefunction from rcentr
           if (l .gt. 5) then
...
!
!
! Calculates parts of wavefunction (from right and left)
...
!           
           idir=1
           triall(0)=meshsize**(l+1)
           triall(1)= (2*meshsize)**(l+1)
           imin=1
