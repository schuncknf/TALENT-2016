...
!------------------------------------------------------------
	...
	! Nodecc= a variable for indexing all different solutions (different N,l..)
     program numerov
      ...
      integer :: Nodecount, numN, numZ, n_rad, st_pos, minN
...
! values read from input_HF.dat
      OPEN(UNIT=9, FILE='input_HF.dat', status='old')
      read(9,'(a,f18.10)') string, strval
      do while (string .NE. '_END_')
        read(9,'(a,f18.10)') string, strval
        if (string .EQ. 'ENEUP') then
            Eup=strval
        else if (string .EQ. 'ENEDO') then
            Edown=strval    
        else if (string .EQ. 'EPSIL') then
            epsil=strval
        else if (string .EQ. 'RBMAX') then
            Rmax=strval
        else if (string .EQ. 'MESHS') then
            meshsize=strval
        else if (string .EQ. 'NUM_Z') then
            numZ=int(strval)
        else if (string .EQ. 'BAS_Z') then
            N_maxZ=int(strval)
        else if (string .EQ. 'NUM_N') then
            numN=int(strval)
        else if (string .EQ. 'BAS_N') then
            N_maxN=int(strval)
!        else if (string .EQ. 'POTEN') then
!            kpot=int(strval)
        else if (string .EQ. 'A_PAR') then
            a=strval
        else if (string .EQ. 'V_VAL') then
            Vvalue=strval
        else if (string .EQ. 'RZERO') then
            rzero=strval
        else if (string .EQ. '_END_') then
            print*, "---input read---"
        end if
      end do
      CLOSE(UNIT=9)
...
!                *(-hb2m* kinden(j)*density(j) &
!                 -density(j)*(density(j)*(-1132.4) + (2+alpha)*23610.4*(density(j)**alpha)/12 + &
!                 density(j)*(+1132.4/2 -23610.4/12*(density(j)**alpha)) &
!                   + alpha*(density(j)**(alpha-1))*(-23610.4*(denZ(j)**2+denN(j)**2)/24)))
!         end do
!         print*, "###############################"        
!         print*, tot_E
!         print*, "###############################" 
...
!
         pre_denZ(:)=denZ(:)
           ...
           if (abs(tot_E-E_before) .lt. 0.001) then
              HF_conv=1
              print*, ":)"
           end if
            E_before=tot_E
