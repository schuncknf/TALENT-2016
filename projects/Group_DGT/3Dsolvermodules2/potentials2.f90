...
!===============================================================
  ...
  !       INTEGER, PARAMETER :: dp = SELECTED_REAL_KIND(12)
         ...
         real (kind=dp) :: rho, rho_q, rho_p, rho_n, alphas
...
! rho_p = proton density
         ...
         bigR= 3.200199d0!rzero*(numN+numZ)**(1.0E0_dp/3.0E0_dp)
...
!-------------------------------------
! WOODS-SAXON + CENTRIFUGAL TERM
           else if(kpot .eq. 2) then
             ferre=1/(1+exp((abs(posiR(i))-bigR)/a))
             vr=-51.*ferre + hb2m*((l*(l+1))/(posiR(i))**2)
...
! SIMPLE SKYRME (manuale 3.3.1) with t0 and t3
          ...
          Uq_sk = rho*(t0s + (2.d0+alphas)*t3s*(rho**alphas)/12.d0) + rho_q*(-t0s/2.d0 -t3s/12.d0*(rho**alphas)) &
                   + alphas*(rho**(alphas-1.0d0))*(-t3s*(rho_p**2+rho_n**2)/24.d0)
          V_cfug=hb2m*((l*(l+1))/(posiR(i))**2)
 !         if(i .eq. 0) V_cfug=0.0E0_dp 
          vr=Uq_sk+V_cfug
