...
!potential function
 ...
 !        INTEGER, PARAMETER :: dp = SELECTED_REAL_KIND(12)
         ...
         real (kind=dp) :: Uq_pot, Wq_pot1,Wq_pot2, V_Cou
...
!
         if(kpot .eq. 0) then
           ...
           if(pos .ge. leftlim .and. pos .le. rightlim) then
             ...
             Wq_pot1=(dble(jj)/2*(dble(jj)/2+1) - l*(l+1)-3/4)/pos*0.44! &
!                    *exp((abs(pos-pos0)-bigR)/a) *rzero**2 !&
!                    /a/pos/(1+exp((abs(pos-pos0)-bigR)/a))**2
             Wq_pot2=(51-33*(numN-numZ)/(numN+numZ))
             vr=Uq_pot+Wq_pot1*Wq_pot2+hb2m*(l*(l+1)/pos**2)
             if(charge .eq. 1) then
               if(abs(pos-pos0) .le. rProt) then
