...
#include <gsl/gsl_sf_legendre_Pl.h>
...
double galag1 (int n, int n1, int n2, int n3, int n4, double m, double w,double r1, double r2, double Vr, double Vs, double Vt, double l, double s1, double s2, double s3, double s4);

double galag2 (int n, int n1, int n2, int n3, int n4, double m, double w,double r1, double r2, double Vr, double Vs, double Vt, double l, double s1, double s2, double s3, double s4);

double twodgalag (int n, int n1, int n2, int n3, int n4, double m, double w, double Vr, double Vs, double Vt,double kappar, double kappas, double kappat, double l, double l, double s1, double s2, double s3, double s4);
...
double fun1 (double kappar, double kappas, double kappat, double Vr, double Vs, double Vt,double r1, double r2,double l)

	{
		...
		res = 0.5*(Vr*exp(-1*kappar*(r1*r1+r2*r2))+Vs*exp(-1*kappas*(r1*r1+r2*r2)))/(2*(r1+0.01)*(r2+0.01))/4;	// The integral of the spatial form-factor next to (1-P_sigma).	
...
double fun1r (double kappar, double kappas, double kappat, double Vr, double Vs, double Vt ,double r1, double r2,double l)

	{
		...
		res = 0.5*(Vr*exp(-1*kappar*(r1*r1+r2*r2))+Vs*exp(-1*kappas*(r1*r1+r2*r2)))/(2*(r1+0.01)*(r2+0.01))/4;	// The integral of the spatial form-factor next to (1-P_r).
		
		return res;
	
	}

double fun2 (double V, double m, double w,double r1, double r2,int n1, int n2, int n3, int n4, double l, double s1, double s2, double s3, double s4,double kappar, double kappas, double kappat)
	
	{
	
		double res,res1, res2, dummy1, dummy2;
		
		dummy1 = galag1(5,n1,n2,n3,n4,m,w,r1,r2,Vr,Vs,Vt,kappar,kappas,kappar,s1,s2,s3,s4);
				
		dummy2 = galag2(5,n1,n2,n3,n4,m,w,r1,r2,Vr,Vs,Vt,kappar,kappas,kappar,s1,s2,s3,s4);		
