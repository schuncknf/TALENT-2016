//  //  //  //  //  //  //  //  //  //  //  //  //  //  //  //
//  HFSolverFast.c                                          //
//  Created by Yuchen Cao on 7/29/16.                       //
//  ****************************************************    //
//  With a proper Two Body Matrix Elements file             //
//  and it's corresponding ID to quantum numbers            //
//  file, this program can solve the Hartree Fock           //
//  energy of a given system of both neutrons               //
//  and protons.                                            //
//  Current TBME matrix only saves                          //
//  neutron data with isospin t = 0.5, the IF statement     //
//  in the TBME IMPORT section controls this. Also,         //
//  change N to equal to NStates, if only deal with         //
//  neutrons N is half of NStates, while NStates is the     //
//  number of Orbit available.                              //
//                                                          //
//  One should be aware that in TBME matrix import,         //
//  I have swapped j & k indices so that in the HF solver   //
//  "nested for loop" there'll be lesser cache misses       //
//  when reading TBME matrix.                               //
//                                                          //
//  In current cases we use Real & Symmetric Hamiltonian,   //
//  this has been improvised in HF solver.                  //
//  //  //  //  //  //  //  //  //  //  //  //  //  //  //  //
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
#define cCrit 1.0*pow(10.0,-7.0)//convergence criteria for eigenvalues
...
#define file0 "TBME/VM-scheme_n5_l2.dat"//"TBME/VM-scheme_n5_l2.dat"
...
int index_n[NStates+1], index_l[NStates+1], index_t[NStates+1];
//RInd(RealIndex) is 1D arrays for storing actual orbit number ID, since we only store neutron TBMEs, we exclude t=-1 TBMEs,thus we want to creat a (e.g.) 20*20*20*20 matrix instead of a 40*40*40*40 matrix, RInd is used to restore Orbit number so that we can obtain n,l quantum numbers together with index_n[],index_l[]. IRnd is the reverse projection.
...
int main()
{
  ...
  while (fgets(line, sizeof(line), fp0))
  {
    if (m>2)
    {
        ...
        //we don't want to waste time accessing memory if TBME =0, we also don't want isospin = -0.5, proton data.
        if (index_t[i] == 1 && index_t[j] == 1 && index_t[k] == 1 && index_t[l] == 1 && ret0 !=0.0)
        {
            //The "correct" way to write TBME is in order of i,j,k,l, if you want to compare with original file.
 ...
 //=========================================== HF SOLVER LOOP ==========================================
  ...
  double spPot;
  double h0;
  double TBME_c,dens_c;//value cache for TBME and density
    while(iter <= maxIter)
    {
        for (i=0; i < N; i++)
        {
            // j only runs from i to N, since Hamiltoninan is symmetric in our case
            for (j=i; j < N; j++)
            {
                spPot = 0;
                for (k=0; k < N ; k++)
                {
                    for (l=0; l < N ; l++)
                    {
                        //we don't need to run the density loop if TBME is zero.
                        TBME_c = TBME[i][j][k][l];
                        if (TBME_c != 0.0)
                        {
                            dens_c = density(k,l, D);
                            if (dens_c != 0.0)
                            {
                                spPot += dens_c * TBME_c;
                            }
                ...
                else if ( i != j && spPot != 0.0)
                {
                    gsl_matrix_set (H, i, j, spPot);
                }
                else
                {
                    gsl_matrix_set (H, i, j, 0);
                }
    ...
    if ( (float)gsl_vector_get(diff,N-1) < (float)cCrit)
    {
        ...
        printf("Eigenvalues converge***\nHartree-Fock Energy = %f MeV\n", eHF);
        //printf("Convergence Criteria is: 10^%d MeV\n", (int)log10(cCrit));
