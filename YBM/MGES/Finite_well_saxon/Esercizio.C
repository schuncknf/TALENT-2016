...
// 				wave_val.push_back(numerov_algorithm(Etrial, wave_val[i], wave_val[i-1], i*h_width - width_box/2.0, potType, l, s));
// 			}
// 
// 		nodecount = 0;
// 		for(int i=1; i<=n_step_width_box; i++){
...
// 		wave_val.push_back(numerov_algorithm(eigenval[5], wave_val[i+1], wave_val[i], i*h_width - width_box/2.0, potType, l, s));
// 		if(i*h_width>24.&&wave_val[i]*wave_val[i-1]<0){
// 			count = i;
// 			break;
// 		}
// 		myfile << i*h_width - width_box/2.0 << "   " << wave_val[i] << endl;
// 	}
...
// 	myfile.close();
    ...
    ofstream ofile;
    ofile.open("potTest.dat");
    for(int i=0; i<=width_box/h_width/2.0; i++){
        ofile << i*h_width << "\t" << potential(i*h_width, potType, l, s) << endl;
    }
