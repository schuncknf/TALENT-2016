...
int main()
{
	...
	for(int isoSpin=0; isoSpin<2; isoSpin++)
	{
		...
		for(int L=0; L<8; L++) // Loop up to L=7 ( i.e. i_13/2) orbitals
		{
				...
				for(int jj=0; jj<nSteps; jj++) // (eMax+eMin)
				{
					...
					if(jj>0)
					{
						...
						if(wfPrev/wfLast < 0)
						{
							...
							if(isoSpin==0) sprintf(filename,"results/proton_%i_%1.4f.dat",L,eigenEng);
							else sprintf(filename,"results/neutron_%i_%1.4f.dat",L,eigenEng);
							...
							if(L==3) sprintf(orbital,"f_%1.0f/2",2*j);
							if(L==4) sprintf(orbital,"g_%1.0f/2",2*j);
							if(L==5) sprintf(orbital,"h_%1.0f/2",2*j);
							if(L==6) sprintf(orbital,"i_%1.0f/2",2*j);
