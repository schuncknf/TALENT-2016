				...
				for(int jj=0; jj<nSteps; jj++) // (eMax+eMin)
				{
					...
					if(jj>0)
					{
						...
						if(wfPrev/wfLast < 0)
						{
							...
							for(int k=0; k<wBox/h; k++) opFile << h*k << "\t" << sqrt(normFac)*wf_val[k] << "\t" << V(k*h, isoSpin, L, spin) << "\t" << woodsSaxon(k*h) << "\t" << centrifugal(k*h,L) << "\t" << spinOrbit(k*h,L,0.5) << "\t" << coulomb(k*h) << endl;
							opFile.close();
							...
							opFile2 << fixed << setprecision(10) << orbital << "\t\t" << eigenEng << endl;
                            
							vector<double> density(length,0.0);

							for(int ii=0;ii<length;++ii)
							{
								double tempWF = sqrt(normFac) * wf_val[ii];
								double rad = (ii+1)*h;
								density[ii] = ((2*j + 1) * tempWF * tempWF /(4*M_PI));
...
//	for(int i=0; i<wBox/h; i++) opFile << h*i << "\t" << totPot.at(i) << "\t" << totProton.at(i) << "\t" << totNeutron.at(i) << endl;
	for(int i=0; i<wBox/h; i++) opFile << h*i << "\t" << totPot[i] << "\t" << totProton[i] << "\t" << totNeutron[i] << endl;
	opFile.close();
...
double normalise(double eigenEng, int isoSpin, int L, int spin)
{
	...
	for(int i=0; i<(wBox/h)-1; i++) sum += h*pow(wfWork[i],2);
