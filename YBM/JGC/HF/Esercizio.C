...
//Routine to obtain neutron density and proton density, and the sum of both
	...
	do{
		...
		for(int i=2; i<n_step_width_box; i++){
			wav_HF.push_back(numerov_algorithm_woods_HF(Etrial, wav_HF[i-1], wav_HF[i-2], i*h_width + h_width, S, L, J, T, U_skyrme_p[i-2], U_skyrme_p[i-1], U_skyrme_p[i]));
		}
		"Calculate densities"
		U_skyrme_p.clear();
		U_skyrme_n.clear();
	...
	}while("condition: compare old total energy with new");
