...
/////////////////////HARTREE FOCK///////////////////////////
	...
	double skyrme(double rho, double rho_q, double rho_p, double rho_n, double rho1, double rho_q1, double rho2, double rho_q2, double r, double tau, double tau_q){
		double d_rho = (rho1-rho)/h_width;
		double d_rhoq = (rho_q1-rho_q)/h_width;
		double d2_rho = (rho2 - 2*rho1 + rho)/pow(h_width,2);
		double d2_rhoq = (rho_q2 - 2*rho_q1 + rho_q)/pow(h_width,2);
		...
		return  rho * ((t0/2.)*(2.+x0) + (2.+_a)*(t3/24.)*(2.+x3)*pow(rho,_a)) +
				...
				1./16.*(3.*t1*(2+x1)-t2*(2+x2))*(d2_rho+2*d_rho/r)-
				1./8.*(t1*(2*x1+1)-t2*(2*x2+1))*tau_q-
				1./8.*(t1*x1+t2*x2)*(d2_rhoq+2*d_rhoq/r);
	}
