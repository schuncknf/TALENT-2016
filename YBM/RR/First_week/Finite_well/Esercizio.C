	...
	for(int node=0; node<4; node++){
		...
		do{
			...
			for(int i=1; i<=n_step_width_box; i++){
				wave_val.push_back(numerov_algorithm_finitewell(Etrial, wave_val[i], wave_val[i-1], i*h_width));
			}
...
// Write results to file with filename
	...
	for(int i=1; i<=n_step_width_box; i++){
		wave_val.push_back(numerov_algorithm_finitewell(eigenval[n], wave_val[i], wave_val[i-1], i*h_width));
