...
real(8),parameter :: pi=3.141592653589793d0 ! hbar: (MeV*s), unit:(MeV),(fm),(s)
real(8),parameter :: t0=-1132.4d0, t3=23610.4d0, x0=0d0, x3=0d0, alpha=1d0
real    :: s_z, j_tot
integer,parameter :: iter=10000
real(8),allocatable :: psi(:), &
     u_nlj_p(:,:), u_nlj_n(:,:),u_nlj(:,:),&
...
real(8) :: E_left, E_right, E, E_hf, E_ke, E_sp, E_hf_new, number, E_pot
real(8) :: x, dx, nmfactor, rms_sq, rms
integer :: i, j, rep, count, spin, l, ex
real(8),allocatable :: energy_n(:), energy_p(:), wave_func_n(:,:), wave_func_p(:,:)
integer :: orbital, orbital_l
...
NAMELIST / output / output_wave_func
...
!write(*,*) 'orbital=', orbital

allocate(psi(0:N), &
     u_nlj_p(0:N,1:orbital), u_nlj_n(0:N,1:orbital),u_nlj(0:N,1:orbital))
...
! initial sp energy
open(10,file='initial_n.dat')
do orbital_l=1,orbital
   read(10,*) energy_n(orbital_l)
end do
close(10)
open(10,file='initial_p.dat')
do orbital_l=1,orbital
   read(10,*) energy_p(orbital_l)
end do
close(10)

! decide initial wave func
u_nlj_p = 0d0
u_nlj_n = 0d0
...
do ex=0,n_max
   do l=0,l_max
      do spin=1,2
         ...
         write(filename_wave_func_p(orbital_l),'(a12,2i2.2,i3.3,a5)') &
              'p082n126nl2j', ex+1, l, nint(2*(l+s_z)), 'p.dat'
         write(filename_wave_func_n(orbital_l),'(a12,2i2.2,i3.3,a5)') &
              'p082n126nl2j', ex+1, l, nint(2*(l+s_z)), 'n.dat'
         open(10,file=filename_wave_func_p(orbital_l))
         open(20,file=filename_wave_func_n(orbital_l))
         read(10,*,end=99)
         read(20,*,end=99)
         do i=0,N
            read(10,*) x, u_nlj_p(i,orbital_l)
            read(20,*) x, u_nlj_n(i,orbital_l)
         end do
         close(10)
         close(20)
99 continue
      ...
      do spin=1,2
         ...
         do i=1,N-2
            ...
            tau_n(i) = tau_n(i) + (2d0*j_tot+1)*( ((4d0*u_nlj_n(i+1,orbital_l)&
                 ...
                 - u_nlj_n(i,orbital_l)/x)**2 + l*(l+1d0)*u_nlj_n(i,orbital_l)**2/x**2&
                 )/(4d0*pi*x**2)
!            tau_n(i) = tau_n(i) + (2d0*j_tot+1)*( ((u_nlj_n(i+1,orbital_l)&
!                 -u_nlj_n(i,orbital_l))/dx&
!                 - u_nlj_n(i,orbital_l)/x)**2 + l*(l+1d0)*u_nlj_n(i,orbital_l)**2/x**2&
!                 )/(4d0*pi*x**2)
         end do
...
do i=1,N-2
   ...
   tau_n(i) = tau_n(i) + Nn_l*( ((4d0*u_nlj_n(i+1,orbital_tmp+1)&
        ...
        - u_nlj_n(i,orbital_tmp+1)/x)**2 + l*(l+1d0)*u_nlj_n(i,orbital_tmp+1)**2/x**2&
        )/(4d0*pi*x**2)
!   tau_n(i) = tau_n(i) + Nn_l*( ((u_nlj_n(i+1,orbital_tmp+1)&
!        -u_nlj_n(i,orbital_tmp+1))/dx&
!        - u_nlj_n(i,orbital_tmp+1)/x)**2 + l*(l+1d0)*u_nlj_n(i,orbital_tmp+1)**2/x**2&
!        )/(4d0*pi*x**2)
end do
rho_n(0) = rho_n(1)
      ...
      do spin=1,2
         ...
         do i=1,N-2
            ...
            tau_p(i) = tau_p(i) + (2d0*j_tot+1)*( ((4d0*u_nlj_p(i+1,orbital_l)&
                 ...
                 - u_nlj_p(i,orbital_l)/x)**2 + l*(l+1d0)*u_nlj_p(i,orbital_l)**2/x**2&
                 )/(4d0*pi*x**2)
         end do
...
do i=1,N-2
   ...
   tau_p(i) = tau_p(i) + Np_l*( ((4d0*u_nlj_p(i+1,orbital_tmp+1)&
        ...
        - u_nlj_p(i,orbital_tmp+1)/x)**2 + l*(l+1d0)*u_nlj_p(i,orbital_tmp+1)**2/x**2&
        )/(4d0*pi*x**2)
end do
rho_p(0) = rho_p(1)
tau_p(0) = tau_p(1)
...
! other part
   ...
   dg_n(:) = 0d0
   do i=0,N
      h_p(i) = V_skyrme_p(i)+ VCO(i*dx, neutron, proton, e)
      h_n(i) = V_skyrme_n(i)
   end do
...
!      print*, E_ke
   ...
   E_hf = (E_ke + E_pot)
   write(*,'(4f20.8)') number, E_hf, E_sp, E_ke
...
if (output_wave_func) then
   do orbital_l=1,orbital
      ...
      write(10,'(a5,3i5)') 'nl2j=', nint(E_data_n(orbital_l,2)),&
      ...
      open(10,file=filename_wave_func_p(orbital_l))
      write(10,'(a5,3i5)') 'nl2j=', nint(E_data_p(orbital_l,2)),&
...
deallocate(psi,u_nlj_p,u_nlj_n,u_nlj)
...
!=========================== potential function ===============================
      ...
      ! VCO
      real (8) function VCO(xy, neutron, proton, e)
      implicit none
      real(kind=dm):: R, xy, AA
      !WoodsSaxon
